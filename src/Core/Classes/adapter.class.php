...
class Adapter{
	 ...
	 */
	public function update($data, $where, $primary_key)
	{
	...
	public function foreign_key_values( $reference ){
		$rows = "";
		$sql = "SELECT REFERENCED_TABLE_NAME, 
						REFERENCED_COLUMN_NAME 
				FROM INFORMATION_SCHEMA.KEY_COLUMN_USAGE 
		...
		$referenced = $this->query($sql);
		if( haveRows( $referenced ) ){
			$prefix_name = str_replace( "_id", "_name", $reference );
			//$prefix_status = str_replace( "_id", "_status", $reference );
			$sql = "SELECT {$reference} AS id, {$prefix_name} AS name FROM {$referenced[0]['REFERENCED_TABLE_NAME']}";
			$rows = $this->query($sql);
		}
		return $rows;
	}
