...
function Error($message)
{
    print '<html><head><title>' .
        $message .
        '</title></head><body style="background-color:#ccc;">';
    print '<pre><h1> Sistema (V 1.0.0)</h1></pre>';
    pr(
        '<span style="color:#f00; font-size:16px;"><strong>' .
            $message .
            '</strong></span>'
    );
    pr(
...
function pageNumber()
{
    ...
    $page = 1;
    if (number($_GET['page']) == 0) {
        if (number($_POST['page']) == 0) {
            $page = 1;
        } else {
            $page = number($_POST['page']);
        }
    } else {
        $page = number($_GET['page']);
    }
    return $page;
}
...
function param($param)
{
    return isset($_GET[$param])
        ? cleanParam($_GET[$param])
        : (isset($_POST[$param])
            ? cleanParam($_POST[$param])
            : '');
}
